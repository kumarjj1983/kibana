...
import { Embeddable, TimeRange, triggerRegistry } from 'ui/embeddable';
import { FILTER_ACTION } from 'ui/embeddable/actions/trigger_registry';
...
export class SearchEmbeddable extends Embeddable<SearchInput, SearchOutput> {
  ...
  constructor(
    { savedSearch, editUrl, $rootScope, $compile }: SearchEmbeddableConfig,
  ...
  private initializeSearchScope() {
    ...
    searchScope.filter = (field, value, operator) => {
      ...
      const filterActions = triggerRegistry.getTrigger(FILTER_ACTION).getCompatibleActions({
        embeddable: this,
        container: this.container,
      });
      filterActions[0].execute({
        embeddable: this,
        actionInput: generateFilters(stagedFilter),
        container: this.container,
      });
    };
